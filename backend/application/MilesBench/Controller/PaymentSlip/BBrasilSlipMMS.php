...
class BBrasilSlipMMS {
	...
	public function generateRemittance(Request $request, Response $response) {
		...
		try {
			...
			$config['codigo_empresa'] = '4685443';
			$config['razao_social'] = 'MMS Viagens LTDA';
			...
			$arquivo->configure(array(
				...
				'numero_convenio' => '3094413',
				'numero_sequencial' => $numeroRemessa,
				'nome_fantasia' => 'MMS Viagens', // seu nome de empresa
				'razao_social'  => 'MMS VIAGENS LTDA',  // sua razão social
				'cnpj'          => '29632355000185', // seu cnpj completo
				'banco'         => $codigo_banco, //código do banco
				'logradouro'    => 'Av. Raja Gabaglia',
				'numero'        => '2000,',
				'bairro'        => 'Bairro Estoril', 
				'cidade'        => 'Belo Horizonte',
				'uf'            => 'MG',
				'cep'           => '30494170',
				'agencia'       => '1614', 
				'agencia_dac'       => '4', 
				'conta'         => '400000', // número da conta
				'conta_dac'     => '5', // digito da conta
				'tipo_cobranca' => '02VIN'
			));
			...
			foreach ($clientes as $key => $value) {
				...
				if($value['paymentType'] == 'Boleto' && $value['billingPeriod'] == 'Diario' && $value['billToReceive']['valueFloat'] > 0 && is_numeric($value['registrationCode']) ) {
					...
					$arquivo->insertDetalhe(array(
						...
						'nosso_numero'      => '3094413' . $this->add_zeros($maxNumber, 10),
			...
			$fileName = "BB".(new \DateTime())->format('d').(new \DateTime())->format('m').$variavel;
			...
			if ($handle) {
				while (($line = fgets($handle)) !== false) {
					if( strrpos($line, 'REMESSA') === false && strrpos($line, '9           ') === false) {
						...
						$string = substr($line, 0, 101) . $tipo_cobranca . substr($line, 106, 53) . substr($line, 159, 14) . $replacing_discount_date . substr($line, 191);
						$line = $string;
					}
					fputs($writing, $line);
				}
