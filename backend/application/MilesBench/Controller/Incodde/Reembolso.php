...
class Reembolso {
    ...
    public function reembolso(Request $request, Response $response) {
        ...
        try {
            ...
            $refund = false;
            $issueDate = $sale->getIssueDate()->format('d/m/Y');
            if($sale->getIssueDate()->modify('+60 day') >= (new \DateTime()) && 
			  ($sale->getStatus() != 'Reembolso Solicitado' && $sale->getStatus() != 'Reembolso Pagante Solicitado' && $sale->getStatus() != 'Reembolso Confirmado' && $sale->getStatus() != 'Cancelamento Nao Solicitado' && $sale->getStatus() != 'Reembolso Pendente' && 
			   $sale->getStatus() != 'Cancelamento Solicitado' && $sale->getStatus() != 'Cancelamento Efetivado' && $sale->getStatus() != 'Cancelamento Pendente' )) {
				$refund = true;
            ...
            $sale->setRefundRequested('true');
            $em->persist($sale);
            $em->flush($sale);

            $env = getenv('ENV') ? getenv('ENV') : 'production';
            if($env == 'production') {
                $content = "<br>Ola,<br><br><b>Nova solicitação de reembolso</b><br>".
                    "Solicitante: <b>" . $dados['login'] . "</b><br><br>".
                    "Email: <b>" . $dados['email'] . "</b><br><br>".
                    "Venda id: <b>" . $sale->getId() . "</b><br>".
                    "Data da venda: <b>" . $issueDate . "</b><br>".
                    "LOCALIZADOR: <b>" . $sale->getFlightLocator() . "</b><br>".
                    "PAX: <b>" . $sale->getPax()->getName() . "</b><br>".
                    "EMBARQUE: <b>" . $sale->getBoardingDate()->format('d/m/Y H:i') . "</b><br><br>".
                    "<br><br>SRM-IT";
            ...
            $message = new \MilesBench\Message();
            $message->setType(\MilesBench\Message::SUCCESS);
            $message->setText('Reembolso confirmado com sucesso, gentileza aguardar!');
            $response->addMessage($message);
        } catch(\Exception $e) {
            $message = new \MilesBench\Message();
            $message->setType(\MilesBench\Message::ERROR);
