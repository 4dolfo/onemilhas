(function () {
    'use strict';
    angular.module('app.saleClientWait', ['angularFileUpload', 'ui.mask', 'ui.select']).controller('ClientWaitCtrl', [
      '$scope', '$rootScope', '$filter', '$modal', 'cfpLoadingBar', 'logger', 'FileUploader', '$http', function($scope, $rootScope, $filter, $modal, cfpLoadingBar, logger, FileUploader, $http) {
        var init;
        var original;
        $scope.clientStatus = ['Pendente', 'Aprovado', 'Bloqueado', 'Reprovado', 'Arquivado', 'Coberto', 'Analise prazo', 'Pendente Liberacao', 'Antecipado/Bloqueado'];
        ...
        $scope.row = '';
        $scope.partners = [{}, {}];
        $scope.filters = [{ name: 'id', label: 'id', check: true},
          ...
          { name: 'countd', label: 'Quantidade de emissão(total)', check: true},
          { name: 'last_month_emission', label: 'Quantidade de emissão(ultimo mês)'},
        ...
        $scope.searchRegistrationCode = function() {
          if($scope.selected) {
            if($scope.selected.registrationCode) {
              $.post("../backend/application/index.php?rota=/searchRegistrationCode", {hashId: $scope.session.hashId, data: $scope.selected, partnerType: 'C'}, function(result){
          ...
          $scope.currentSuggestions = $filter('filter')($scope.suggestions, $scope.selected.registrationCode);
        };
        ...
        $scope.setSelected = function() {
          // if(!$scope.main.dealer || $scope.main.internCommercial) {
            ...
            if($scope.selected.last_emission) {
              $scope.selected.last_emission = $rootScope.findDate($scope.selected.last_emission);
            }
            if($scope.selected.birthdate) {
              $scope.selected.birthdate = $rootScope.findDate($scope.selected.birthdate);
            }
            if($scope.selected.registerDate) {
              $scope.selected.registerDate = $rootScope.findDate($scope.selected.registerDate);
            }
            ...
            $.post("../backend/application/index.php?rota=/loadPartnersClient", { hashId: $scope.session, data: $scope.selected}, function(result){
              $scope.partners = jQuery.parseJSON(result).dataset;
              for(var i in $scope.partners){
                if($scope.partners[i].birthdate != "")
                  $scope.partners[i].birthdate = $rootScope.findDate($scope.partners[i].birthdate);
              }
            ...
            $.post("../backend/application/index.php?rota=/loadSalePlans", {hashId: $scope.session, data: $scope.selected}, function(result){
              $scope.SalePlans = jQuery.parseJSON(result).dataset;
            });
  
            $.post("../backend/application/index.php?rota=/loadCommercialProgress", {hashId: $scope.session, data: $scope.selected}, function(result){
            ...
            $.post("../backend/application/index.php?rota=/loadClientAnalisys", { hashId: $scope.session.hashId, data: $scope.selected }, function(result){
              $scope.finnacialInfo = jQuery.parseJSON(result).dataset;
              $scope.$digest();
            });
  
            $.post("../backend/application/index.php?rota=/loadClientDocs", { hashId: $scope.session.hashId, data: $scope.selected }, function(result){
              $scope.clientDocs = jQuery.parseJSON(result).dataset;
              $scope.$digest();
            });
  
            $.post("../backend/application/index.php?rota=/loadClientNotifications", { data: $scope.selected }, function(result){
        ...
        $scope.loadClientRegistrionUpdates = function() {
          $.post("../backend/application/index.php?rota=/client/loadClientRegistrionUpdates", { data: $scope.selected }, function(result){
            $scope.clientRegistrationUpdates = jQuery.parseJSON(result).dataset;
            $scope.$digest();
          });
        };
        ...
        $scope.setStatusNotification = function(not) {
          $.post("../backend/application/index.php?rota=/setStatusNotification", { data: $scope.selected, notification: not }, function(result){
              logger.logSuccess(jQuery.parseJSON(result).message.text);
              $.post("../backend/application/index.php?rota=/loadClientNotifications", { data: $scope.selected }, function(result){
                $scope.clientNotifications = jQuery.parseJSON(result).dataset;
                $scope.$digest();
        ...
        $scope.registerLog = function() {
          $.post("../backend/application/index.php?rota=/saveCommercialProgress", {hashId: $scope.session.hashId, data: $scope.selected, progress: $scope.newProgress}, function(result){
              logger.logSuccess(jQuery.parseJSON(result).message.text);
              $scope.newProgress = {};
              $.post("../backend/application/index.php?rota=/loadCommercialProgress", {hashId: $scope.session, data: $scope.selected}, function(result){
                  $scope.ClientLog = jQuery.parseJSON(result).dataset;
        ...
        $scope.loadIssuers = function() {
          $.post("../backend/application/index.php?rota=/loadIssuers", { data: $scope.selected }, function(result){
        ...
        $scope.saveClient = function() {
            ...
            if($scope.selected.name.length > 40 && $scope.authorized == false ) {
              $scope.openAutorization();
              return logger.logError('Nome da agencia nao deve ser superior a 40 caracteres');
            }
            ...
            cfpLoadingBar.start();
            $.post("../backend/application/index.php?rota=/checkRegister", {hashId: $scope.session.hashId, data: $scope.selected, partners: $scope.partners}, function(result){
              if (jQuery.parseJSON(result).message.type == 'S'){
        ...
        $scope.copyAddress = function() {
          ...
          $scope.selected.adressComplementFinnancial = $scope.selected.adressComplement;
          $scope.selected.adressDistrictFinnancial = $scope.selected.adressDistrict;
        ...
        $scope.loadClient = function() {
          $scope.newUpload = false;
          cfpLoadingBar.start();
          $.post("../backend/application/index.php?rota=/loadClientWait", { page: $scope.currentPage, numPerPage: $scope.numPerPage, searchKeywords: $scope.searchKeywords, order: $scope.searchOrder, orderDown: $scope.searchOrderDown, data: $scope.filter }, function(result){
        ...
        $scope.mailOrder = function() {
          ...
          $.post("../backend/application/index.php?rota=/mailOrder", {hashId: $scope.session.hashId, data: $scope.wemail, attachment: file, type: 'COMERCIAL', emailType: $rootScope.emailType}, function(result){
            if (jQuery.parseJSON(result).message.type == 'S'){
              logger.logSuccess(jQuery.parseJSON(result).message.text);
              $scope.mail = false;
        ...
        $scope.setDocumentStatus = function(doc) {
          $.post("../backend/application/index.php?rota=/setDocumentStatus", { hashId: $scope.session.hashId, data: $scope.selected, doc: doc }, function(result){
            logger.logSuccess(jQuery.parseJSON(result).message.text);
            $.post("../backend/application/index.php?rota=/loadClientDocs", { hashId: $scope.session.hashId, data: $scope.selected }, function(result){
              $scope.clientDocs = jQuery.parseJSON(result).dataset;
              $scope.$digest();
        ...
        $scope.returnLoginText = function(issuer) {
          let text = "<p class='MsoNormal'><span>Seguindo com processo de ativação segue dados para acesso ao Busca Ideal.<span></span></span></p>"+
                  "<p class='MsoNormal'><span>&nbsp;</span>Seu acesso ao Busca Ideal é o seguinte:</p>"+
                  "<p class='MsoNormal'><span></span></p>"+
                  "<p class='MsoNormal'><span>&nbsp;</span></p>"+
                  "<ul type='disc'>"+
                    "<li class='MsoNormal'><span>Usuário: <b>" + issuer.name + "</b></span><span><span></span></span></li>"+
                    "<li class='MsoNormal'><span>Senha: <b>" + issuer.name.split('.')[1] + "123</b></span><span><span></span></span></li>"+
                  "</ul>"+
                  "<p class='MsoNormal'><span><span class='m_-2386000081486288919gmail-apple-converted-space'><br></span></span></p>"+
                  "<p class='MsoNormal'><span style='color: rgb(46, 116, 181);'>Link Busca Ideal:<a href='https://novo.buscaideal.com/#/login'>https://novo.buscaideal.com/#/login</a><a href='https://novo.buscaideal.com/#/login' target='_blank'></a></span><br></p>"+
                  "<p class='MsoNormal'><span></span></p>"+
                  "<br>"+
                  ...
                  "<p class='MsoNormal'><span>&nbsp;</span><span></span></p>"+
                  "<p class='MsoNormal'><b><i>Havendo interesse na criação de outro (s) usuário (s), favor responder este e-mail com os dados abaixo:</i></b><br></p>"+
                  "<ul type='disc'>"+
                    "<li class='MsoNormal'><b><i><span>Nome completo:</span></i></b><span></span></li>"+
                    "<li class='MsoNormal'><b><i><span>CPF:&nbsp;</span></i></b><span></span></li>"+
                    "<li class='MsoNormal'><b><i><span>E-mail</span></i></b></li>"+
                  "</ul>"+
                  "<div><b><i><br></i></b></div>"+
          ...
          if($scope.selected.subClient) {
            ...
            text = text.replace("http://buscaideal.com", "http://busca.voeb2b.com.br");
          }
          return text;
        };
  
        $scope.returnActivivationText = function() {
          let text =  "  <p style='color: rgb(34, 34, 34);background-color: rgb(255, 255, 255);'><span>Caro parceiro nosso cadastro foi concluído, algumas informações sobre procedimentos e contatos estão no anexo desse e-mail.</span><span><br><br><span>Atender a " + $scope.selected.name +             " será um grande prazer, gostaria de desde já nos colocar à disposição para o que precisar, sejam duvidas, criticas, sugestões ou qualquer outra demanda, abaixo seguem nossos telefones de contato.</span></span></p>"+
                    ...
                    "<p style='color: rgb(34, 34, 34);background-color: rgb(255, 255, 255);'><span></span></p>"+
                    "<p style='color: rgb(34, 34, 34);background-color: rgb(255, 255, 255);'><span>&nbsp;</span></p>"+
                    "<p style='color: rgb(34, 34, 34);background-color: rgb(255, 255, 255);'><span>Fixo: &nbsp; &nbsp; &nbsp; &nbsp; (31) 3546-0800</span></p>"+
        ...
        $scope.getFild = function(client, fild) {
          if(new Date(client[fild]) == 'Invalid Date' || typeof client[fild] == "number" || client[fild] == "" || client[fild].length <= 9 || fild == 'name') {
            return client[fild];
          } else {
            return $filter('date')($rootScope.findDate(client[fild]),'dd/MM/yyyy');
          }
        };
        ...
        init = function() {
          ...
          if($scope.main.dealer && ($scope.main.id == 22091 || $scope.main.id == 37362 || $scope.main.id == 25112 || $scope.main.id == 28341)) {
            $scope.filters = [
            { name: 'name', label: 'Nome', check: true },
          ...
          if(!$scope.main.dealer) {
            $.post("../backend/application/index.php?rota=/loadClientSuggestions", {}, function(result){
        ...
        $scope.generateHash = function(){
          $.post("../backend/application/index.php?rota=/generateHashClient", {}, function(result){
      ...
      '$scope', '$rootScope', '$modalInstance', 'filter', 'states', 'dealers', function($scope, $rootScope, $modalInstance, filter, states, dealers) {
        
        $scope.states = states;
        $scope.dealers = dealers;
        $scope.clientStatus = ['Pendente', 'Aprovado', 'Bloqueado', 'Reprovado', 'Arquivado', 'Analise prazo', 'Pendente Liberacao', 'Antecipado/Bloqueado'];
        $scope.clientPayments = ['Boleto', 'Antecipado'];
        $scope.filter = filter;
      ...
      '$scope', '$rootScope', '$modal', '$log', 'logger', function($scope, $rootScope, $modal, $log, logger) {
        ...
        $scope.saveIssuer = function() {
          $.post("../backend/application/index.php?rota=/saveIssuer", {hashId: $scope.session.hashId, data: $scope.issuer_selected, client: $scope.$parent.selected}, function(result){
            logger.logSuccess(jQuery.parseJSON(result).message.text);
      ...
      '$scope', '$rootScope', '$modalInstance', 'logger', 'issuer_selected', function($scope, $rootScope, $modalInstance, logger, issuer_selected) {
        ...
        $scope.remove = function() {
          $.post("../backend/application/index.php?rota=/removeIssuer", {hashId: $rootScope.hashId, data: $scope.issuer_selected}, function(result){
            logger.logSuccess(jQuery.parseJSON(result).message.text);
      ...
      '$scope', '$rootScope', '$modalInstance', 'logger', 'clientRegistrationUpdates', 'selected', function($scope, $rootScope, $modalInstance, logger, clientRegistrationUpdates, selected) {
        ...
        $scope.reject = function(registration) {
          ...
          $.post("../backend/application/index.php?rota=/client/rejectRegistrationUpdate", { data: registration }, function(result){
            logger.logSuccess(jQuery.parseJSON(result).message.text);
            $scope.cancel();
          });
        };
        ...
        $scope.accept = function(registration) {
          registration.id = $scope.selected.id;
          $.post("../backend/application/index.php?rota=/client/acceptRegistrationUpdate", { data: registration }, function(result){
            logger.logSuccess(jQuery.parseJSON(result).message.text);
            $scope.cancel();
          });
        };
